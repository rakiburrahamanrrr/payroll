...
function GetDataByIDAndType($ID, $Type) {
  global $db;
	...
	if ( $Type == 'admin' ) {
		...
		$query = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "employees` WHERE `employee_id` = $ID LIMIT 0, 1");
		if ( $query ) {
			if ( mysqli_num_rows($query) == 1 ) {
				$userData = mysqli_fetch_assoc($query);
			}
...
function GetEmployeeLWPDataByEmpCodeAndMonth($EmpCode, $month) {
    ...
    $query = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "leaves` WHERE `employee_id` = '$EmpCode' AND `leave_type` = 'Leave Without Pay' AND `leave_status` = 'approve'");
    if ( $query ) {
        if ( mysqli_num_rows($query) > 0 ) {
...
function Send_Mail($subject, $message, $toName, $toMail, $fromName = FALSE, $fromMail = FALSE,  $cc = FALSE, $bcc = FALSE, $attachment = FALSE, $debug = FALSE) {
    ...
    if ( is_array($attachment) && count($attachment) > 0 ) {
        for ( $i = 0; $i < count($attachment); $i++ ) {
            if (isset($attachment[$i]['src']) && isset($attachment[$i]['name'])) {
